...
data_ = pd.read_csv('Datasets/311_dataset.txt', delimiter = ' ')
data_.to_csv('dataset.csv', index = None)
data_.columns = ['Timestamp', 'File_ID', "File_Size"]
...
for j in tqdm(range(2)):
    ...
    for i in tqdm(range(NumSeq)):
        ...
        if i==past+future:
            ...
            if j==0:
                mse1 = mse1 + math.exp(e)
                w1 = w1 + math.exp(-neita*mse1)
            else:
                mse2 = mse2 + math.exp(e)
                w2 = w2 + math.exp(-neita*mse2)
...
print(w1)
print(w2)
print(float(w1/(w1+w2)))
print(float(w2/(w1+w2)))